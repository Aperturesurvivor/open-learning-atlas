...
Implementation evidence includes the baseline [desktop](implementation/desktop.png)
and [mobile](implementation/mobile.png) views plus direct relationship-record
views on [desktop](implementation/desktop-relationship.png) and
[mobile](implementation/mobile-relationship.png).
...
| Inspector | Title, type/review state, overview, relationships, provenance, open data, and share action. | All are present. Node inspection exposes stable identity, confidence, assessability, scope, and review notes. Expanding any relationship exposes its exact direction, rationale, conditions, claim basis, dependency kind, evidence strength, confidence, status, revision, provenance citations, stable edge ID, and permanent link. | Match plus trust detail |
| Review handoff | Candidate status should lead to an accountable process rather than imply machine approval. | Every territory inspector provides a release-pinned **Start review** action to the attributed GitHub review form. | Added governance affordance |
...
| Status strip | Desktop summarizes graph scale, openness, date, and license. | Counts are computed from the release: 1,213 records and 2,955 relationships, plus update and license. | Match with canonical values |
